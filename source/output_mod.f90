...
!                 print*, grid(iG)%ionDen(cellPUsed, 1, 2), grid(iG)%Ne(cellPUsed),  grid(iG)%Te(cellPUsed)
              ...
              elseif (grid(iG)%lgBlack(cellPUsed)==0 ) then
                 ...
                 if (lgDebug) linePacketsUsed = grid(iG)%linePackets(cellPUsed, :)
...
!if (.not.grid(iG)%lgBlack(cellPUsed)<1) then
!   print*, "BLACK CELL!"
!   print*, "BLACK CELL!", cellPUsed
!   print*, ionDenUsed(1,2), NeUsed, TeUsed
!end if
                 ...
                 if (present(extMap) .and. .not.lgVoronoi) then
                    ...
                    do iup = 30, ilow+1, -1
                       do ilow = 2, min0(8, iup-1)
                          HIRecLines(iup, ilow) = &
                               & HIRecLines(iup, ilow)*10.**(-(cMap(cellPUsed)*flam(iCount)&
                               & +cMap(cellPUsed)))
                          iCount = iCount + 1 
                    ...
                    do l = 1, 34
                       HeIRecLines(l) = HeIRecLines(l)*10.**& 
                            & (-(cMap(cellPUsed)*flam(iCount)+cMap(cellPUsed)))
                       iCount = iCount + 1
                    end do
                    ...
                    do iup = 3, 30
                       do ilow = 2, min0(16, iup-1)
                          HeIIRecLines(iup,ilow) = HeIIRecLines(iup,ilow)*& 
                               & 10.**(-(cMap(cellPUsed)*flam(iCount)+cMap(cellPUsed)))
                          iCount = iCount + 1
                       end do
                    end do
...
itot = itot+1
voltot = voltot+dV
hitot = hitot+ HIRecLines(4,2)

if (nIterateMC == 2) print*, 'cpu', cellPUsed, HIRecLines(4,2), hitot, dV, HdenUsed
                 ...
                 if (lgDebug) then
                    ...
                    do l = 1, nbins
                       totEscapedPackets = totEscapedPackets + &
                            & grid(iG)%escapedPackets(cellPUsed,l,0)
                    end do
                 ...
                 print*, '! outputGas: insane lgBlack'
                 stop
              end if
              end if
        ...
        print*, 'OUTPUTSTATS ITOT HITOT VOLTOT: ', itot, hitot, voltot        
...
!           end if
        ...
        do iAb = 0, nAbComponents
           ...
           write(27, *) "Hbeta [E36 erg/s]:     ", HbetaVol(iAb)
           ...
           if (lgDebug .and. .not.lgVoronoi) then
...
!if (nIterateMC == 2) print*, 'hebtaclac: ', log10Te, Hbeta, NeUsed, ionDenUsed(elementXref(1),2), elemAbundanceUsed(1)
...
!        end do
        ...
        tte = TeUsed
        if (tte <300.) tte = 300.
        if (tte >30000.) tte = 30000.
        nne = NeUsed
        if (nne < 100.) nne = 100.
        if (nne > 1.e14) nne = 1.e14

        call hlinex(4,2,tte,nne,fh,2)
        hb = fh
        do ilow = 2, 8
           do iup = 30, ilow+1, -1
              call hlinex(iup,ilow,tte,nne,fh,2)              
      ...
      do iG = 1, nGrids
       do freq=1,nbins
          if (.not.lgEcho .and. .not.lgNosource) then
             ...
             do ijk = 1, grid(iG)%nCells

                if (.not.lgEcho) then
                   if (lgNosource) then ! exclude source if requested                     
                      if (lgVoronoi) then
                         print*, "! writeSED: [warning] noSource option not considered in Voronoi grids - ignoring"
                      else
                         do ic=1,nStars
