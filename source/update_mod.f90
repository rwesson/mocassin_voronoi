...
module update_mod
    ...
    contains
      ...
      subroutine updateCell(grid, cellP)
        ...
        integer, intent(in)            :: cellP   ! cell indexes in the active array
...
!        cellP = grid%active(xP, yP, zP)
        ...
        if (lgEcho) then
           if (grid%echoVol(cellP).eq.0.0) then
...
!if (cellP == 1 .and. elem ==1 .and. ion ==1 ) print*, nuArray(j), j, nPhotoSte(elem,ion), grid%JSte(cellP,j), phXSec
...
!                         & nIterateT, " steps. (cell, T)", cellP,TeUsed
...
!                     & nIterateT, " steps. (cell, T)", xP,yP,zP,TeUsed, grid%abFileIndex(grid%active(xP,yP,zP))
...
!                   photoelHeat_d(ns,na) = photoelHeat_d(ns,na)+Qa*photFlux*EY*hcRyd/Pi
        ...
        subroutine thermBalance(heatInt, coolInt)
            ...
            ! find the N(H+)
            Np = grid%ionDen(cellP,elementXref(1),2)*grid%elemAbun(grid%abFileIndex(cellP),1)
            ...
            ! find N(He++)
            Np =  grid%ionDen(cellP,elementXref(2),3)*grid%elemAbun(grid%abFileIndex(cellP),2)
...
!print*, '! ionBal: ', cellP, nIterateX, nPhotoSte(1,1)
          ...
          NeUsed = 0.
            ...
            if (NeUsed==0. .and.lgTalk) print*, '! ionBalance [warning]: cell ', cellP, &
                 &'; NeUsed = ',  NeUsed
...
!print*, cellP, nIterateX
!print*, ionDenUsed(elementXref(1),1), HIOld
!print*, ionDenUsed(elementXref(2),1), HeIOld
!print*, ionDenUsed(elementXref(2),2), HeIIOld
!print*, deltaHI, deltaHeI, deltaHeII, limit
...
!                    &maxIterateX, " steps. Finishing up...", cellP, nphotoste(1,1)
            ...
            ! this was added to help MPI implementation
            NeTemp(cellP) = NeUsed
!print*, cellP, "! ionBalance: [warning] convergence  reached after ", &
!                    &nIterateX, " steps. Finishing up...", nphotoste(1,2)
            ...
            do nS = 1, nSpeciesPart(nspU)
               ...
               do ai = 1, nSizes
                  ...
                  if (lgGas .and. convPercent>=resLinesTransfer .and. (.not.lgResLinesFirst) .and. &
                     ...
                     dustHeatingBudget(grid%abFileIndex(cellP),0) = dustHeatingBudget(grid%abFileIndex(cellP),0)+&  
               ...
               do imul = 1, resLine(iL)%nmul
                  if (resLine(iL)%elem==1) then
                     ...
                     if ( resLine(iL)%ion == 1 .and. resLine(iL)%moclow(imul)==1 .and. resLine(iL)%mochigh(imul)==2 ) then
                        ...
                        Gline = Gline + 10**(-0.897*log10(TeUsed) + 5.05)*grid%elemAbun(grid%abFileIndex(cellP),1)&
