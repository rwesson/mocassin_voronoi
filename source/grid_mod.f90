              ...
              do ix = 1, grid(iG)%nx
                 do iy = 1, grid(iG)%ny
                    do iz = 1, grid(iG)%nz
                       ...
                       grid(iG)%Volume(ix,iy,iz) = getVolume(grid(iG), ix, iy, iz)
...
!
      ...
      subroutine setMotherGrid(grid)
        ...
        integer, pointer                  :: activeRTemp(:,:) ! temporary Hden
...
!          print*, 'b'
          ...
          if (lgGas) then
           ...
           do i = 1, grid%nx
              do j = 1, yTop
                 do k = 1, grid%nz
                    grid%echoVol(grid%active(i,j,k))=0. ! initialize
                    if (grid%active(i,j,k)>0) then
                       ...
                       if (lgGas) then
                          ...
                          do elem = 1, nElements
                             gasCell = gasCell + grid%elemAbun(grid%abFileIndex(grid%active(i,j,k)),elem)*&
                                  & aWeight(elem)*amu
                             totalMass = totalMass + &
                                  & grid%Hden(grid%active(i,j,k))*dV*grid%elemAbun(grid%abFileIndex(grid%active(i,j,k)),elem)*&
                                  & aWeight(elem)*amu
                          end do
                       end if
...
!
                       if (lgEcho) then
                          grid%echoVol(grid%active(i,j,k))=vEcho(grid,i,j,k,echot1,echot2,vol)
...
!print*, nsp, 'here!'
         ...
         subroutine setSubGrids(grid)
           ...
           real, pointer                  :: HdenTemp(:,:,:) ! temporary Hden
           real, pointer                  :: abFileIndexTemp(:,:,:) ! temporary Hden
...
!print*, x,y,z, HdenTemp(ix,iy,iz), NdustTemp(ix,iy,iz), dustAbunIndexTemp(ix,iy,iz)
                          ...
                          if (lgMultiDustChemistry) then
                             ...
                             if (lgGas .and. lgDust) then
                                read(72, *) x,y,z, HdenTemp(ix,iy,iz), NdustTemp(ix,iy,iz),&
                                     &abFileIndexTemp(ix,iy,iz), dustAbunIndexTemp(ix,iy,iz)
                             else if (.not.lgGas .and. lgDust) then
                             ...
                             if (lgGas .and. lgDust) then
                                read(72, *) x,y,z, HdenTemp(ix,iy,iz), NdustTemp(ix,iy,iz),abFileIndexTemp(ix,iy,iz)
                             else if (lgGas .and. .not. lgDust) then
...
!
                          if (lgEcho) then
                             grid(iG)%echoVol(grid(iG)%active(ix,iy,iz))=vEcho(grid(iG),ix,iy,iz,echot1,echot2,vol)
                             echoVolume = echoVolume + vol 
                          endif
...
! BE 28.4.14
         ...
         starPosition(i)%x = starPosition(i)%x
         starPosition(i)%y = starPosition(i)%y
         starPosition(i)%z = starPosition(i)%z
