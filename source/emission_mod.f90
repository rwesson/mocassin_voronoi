...
!print*, 'e'
        if (.not.lgVoronoi) then
           dV = grid%Volume(grid%activeR(1, cellPUsed), grid%activeR(2, cellPUsed),grid%activeR(3, cellPUsed))
        else
!print*, '1'
!print*, cellPUsed
!print*, grid%activeRV(cellPUsed)
...
!print*, 'i'
    contains
      ...
      subroutine initResLinePackets()
              ...
              if (resLine(iRes)%elem==1) then
                 if ( resLine(iRes)%ion == 1 .and. resLine(iRes)%moclow(imul)==1 &
                    ...
                    lineIntensity = lineIntensity+10**(-0.897*log10(TeUsed) + 5.05)* &
                         ...
                         &  NeUsed*grid%Hden(cellPused)*dV*&
                         & (1.-grid%fEscapeResPhotons(cellPUsed, iRes))
        ...
        grid%resLinePackets(cellPUsed) = nint(lineIntensity*1.e-16/(Lstar(1)/Nphotons(1)))
...
&                                  ionDenUsed(elementXref(elem), ion+1) * grid%elemAbun(abFileUsed,elem)            
...
&                                      iup        ! pointer to upper level
        ...
        Hbeta = Hbeta*NeUsed*ionDenUsed(elementXref(1),2)*grid%elemAbun(abFileUsed,1)
        ...
        HIRecLines(15, 8) =HIRecLines(15, 8) + grid%elemAbun(abFileUsed,1)*&
        ...
        HeII4686 = HeII4686*NeUsed*grid%elemAbun(abFileUsed,2)*ionDenUsed(elementXref(2),3)
        ...
        elseif(denint==3) then
           do i = 1, 34
              ...
              if (.not.(HeIRecLines(i) >= 0..and. HeIRecLines(i) <= 1.e30)) then
                 print*, '! recLinesEmission: insanity in HeIRecLines calculation [i, denint, T4, NeUsed]', i, denint, T4, NeUsed
                 print*, ' HeIRecLineCoeff(i,3,:) ', HeIRecLineCoeff(i,3,:)
                 stop
              end if
           end do
        end if
        
        HeIRecLines=HeIRecLines*NeUsed*grid%elemAbun(abFileUsed,2)*ionDenUsed(elementXref(2),2)
...
&                          :: HeIILymanNu       ! HeII Lyman lines freq. [Ryd]          
        ...
        do i = 1, nbins
           ...
           if (.not.(normHI>=0. .and.  normHeI>= 0. .and. normHeII>=0.)) then 
              ...
              print*, i, normHI, normHEI, normHeII, emissionHI(i),  emissionHeI(i), emissionHeII(i)
              stop             
           end if
        ...
        ! rec coeff (see above). 
        correction = alpha2tS * &
             & NeUsed*grid%elemAbun(abFileUsed,2)*&
        ...
        HeIILyman(4) = HeIILyman(4)*NeUsed*ionDenUsed(elementXref(2),3)*grid%elemAbun(abFileUsed,2)
        ...
        do iup = 3, 15
            do ilow = 2, min0(8, iup-1)
               if (.not.HIRecLines(iup, ilow) >= 0.) then
                  print*, ' insanity in HIRecLines, iup, ilow'
                  print*, HIRecLines(iup, ilow), iup, ilow
                  stop
               end if
        ...
        do iup = 3, 30
            do ilow = 2, min0(16, iup-1)
               if (.not.HeIIRecLines(iup, ilow) >= 0.) then
                  print*, ' insanity in HeIIRecLines, iup, ilow'
                  print*, HeIIRecLines(iup, ilow), iup, ilow
                  stop
               end if
        ...
        ! HeI singlets
        do i = 1, 34
           if (.not.HeIRecLines(i) >= 0.) then
              print*, ' insanity in HeIRecLines, i'
              print*, HeIRecLines(i), i
              stop
           end if
                 ...
                 do ilow = 1, nForLevelsLarge
                    do iup = 1, nForLevelsLarge
                       if (forbiddenLinesLarge(ilow, iup) > 1.e-35)  then 
                          ...
                          if (.not.forbiddenLinesLarge(ilow, iup) >= 0.) then
                             print*, ' insanity in forbiddenLinesLarge ilow, iup'
                             print*, forbiddenLinesLarge(ilow, iup), ilow, iup
                             stop
                          end if
                 ...
                 do ilow = 1, nForLevels
                    do iup = 1, nForLevels
                       if (forbiddenLines(elem, ion, ilow, iup) > 1.e-35)  then 
                          ...
                          if (.not.forbiddenLines(elem, ion, ilow, iup) >= 0.) then
                             print*, ' insanity in forbiddenLines ilow, iup'
                             print*, forbiddenLines(elem, ion, ilow, iup), elem, ion, ilow, iup
                             stop
                          end if
        ...
        ! now compute the fraction line photons

        normalize = normalize + grid%totalLines(cellPUsed)
            ...
            do jdir = 0,ndiry
               ...
               do kdir = 0,ndirz
                  ...
                  do icount = 1, safeLimit
                     ...
                     if (cellP > 0 ) then
                        compoP = grids(gPin)%abFileIndex(grids(gP)%active(xP,yP,zP))
