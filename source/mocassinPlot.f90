...
!                   if (iCell > 0) then
       ...
       do iCell = 1, nCellsUp
          ...
          if (icell >0 .and. grid3D(iG)%lgBlack(iCell) == 0) then
             ...
             abFileUsed      = grid3D(iG)%abFileIndex(iCell)
              ...
              if (lgDataAvailable(elem, ion)) then
                 ...
                 if (elem == 26 .and. ion ==2) then
                    ...
                    flinePlotLarge(:, :) =flinePlotLarge(:,:)*&
                         &grid3D(iG)%elemAbun(abFileUsed,elem)*&
                         & ionDenUsed(elementXref(elem), ion)
                 else
...
!        Hbeta = 10**(-0.870*log10Te + 3.57)
        Hbeta = Hbeta*NeUsed*ionDenUsed(elementXref(1),2)*grid3D(iG)%elemAbun(abFileUsed,1)
        ...
        HIRecLines(30, 8) =HIRecLines(30, 8) + grid3D(iG)%elemAbun(abFileUsed,1)*&
        ...
        HeII4686 = HeII4686*NeUsed*grid3D(iG)%elemAbun(abFileUsed,2)*ionDenUsed(elementXref(2),3)
        ...
        HeIRecLines=HeIRecLines*NeUsed*grid3D(iG)%elemAbun(abFileUsed,2)*ionDenUsed(elementXref(2),2)
